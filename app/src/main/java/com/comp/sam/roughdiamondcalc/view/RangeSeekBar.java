package com.comp.sam.roughdiamondcalc.view;
...
public class RangeSeekBar extends View {
    ...
    public void setOnRangeSeekbarChangeListener(OnRangeSeekbarChangeListener onRangeSeekbarChangeListener) {
        ...
        if (this.onRangeSeekbarChangeListener != null) {
            this.onRangeSeekbarChangeListener.valueChanged(getSelectedMinValue(), getSelectedMaxValue(), false);
        }
    }
    ...
    public void apply() {
        ...
        if (onRangeSeekbarChangeListener != null) {
            onRangeSeekbarChangeListener.valueChanged(getSelectedMinValue(), getSelectedMaxValue(), false);
        }
    }
    ...
    protected void setupBar(final Canvas canvas, final Paint paint, final RectF rect) {
        rect.left = barPadding + (thumbDiameter * 0.5f);
        rect.top = 0.5f * (getHeight() - barHeight);
        rect.right = getWidth() - barPadding - (thumbDiameter * 0.5f);
    ...
    protected void setupHighlightBar(final Canvas canvas, final Paint paint, final RectF rect) {
        rect.left = normalizedToScreen(normalizedMinValue) + (getThumbWidth() / 2) + barPadding;
        rect.right = normalizedToScreen(normalizedMaxValue) + (getThumbWidth() / 2) - barPadding;
    ...
    protected void setupLeftThumb(final Canvas canvas, final Paint paint, final RectF rect) {
        ...
        rectLeftThumb.left = normalizedToScreen(normalizedMinValue) + barPadding;
        rectLeftThumb.right = Math.min(rectLeftThumb.left + (getThumbWidth() / 2) + barPadding, getWidth());
        rectLeftThumb.top = (getHeight() - thumbDiameter) * 0.5f;
        rectLeftThumb.bottom = (getHeight() + thumbDiameter) * 0.5f;
    ...
    protected void setupRightThumb(final Canvas canvas, final Paint paint, final RectF rect) {
        ...
        rectRightThumb.left = normalizedToScreen(normalizedMaxValue) - barPadding;
        rectRightThumb.right = Math.min(rectRightThumb.left + (getThumbWidth() / 2) + barPadding, getWidth());
        rectRightThumb.top = (getHeight() - thumbDiameter) * 0.5f;
        rectRightThumb.bottom = (getHeight() + thumbDiameter) * 0.5f;
    ...
    protected int getMeasureSpecHeight(int heightMeasureSpec) {
        int height = Math.round((barPadding * 2) + thumbDiameter);
    ...
    private boolean isInThumbRange(float touchX, double normalizedThumbValue) {
        ...
        Log.d("isInThumb", " x " + x + " left " + left + " ri " + right + " pad " + barPadding);
    ...
    public RangeSeekBar setMinSeekValue(double value) {
        setNormalizedMinValue(value);
        if(onRangeSeekbarChangeListener!=null)
            onRangeSeekbarChangeListener.valueChanged(getSelectedMinValue(),getSelectedMaxValue(),false);
        if (onRangeSeekbarFinalValueListener != null)
            onRangeSeekbarFinalValueListener.finalValue(getSelectedMinValue(), getSelectedMaxValue(), false);
        return this;
    }

    public RangeSeekBar setMaxSeekValue(double value) {
        Log.d("setMaxView",String.valueOf(value));
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (pressedThumb != null) {
                    ...
                    if (onRangeSeekbarChangeListener != null) {
                        onRangeSeekbarChangeListener.valueChanged(getSelectedMinValue(), getSelectedMaxValue(), true);
                    }
                }
                break;
            case MotionEvent.ACTION_UP:
                if (mIsDragging) {
                    ...
                    if (onRangeSeekbarFinalValueListener != null) {
                        onRangeSeekbarFinalValueListener.finalValue(getSelectedMinValue(), getSelectedMaxValue(), true);
                    }
                } else {
